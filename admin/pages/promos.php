...
<head>
  ...
  <style>
    /* ── Toolbar ── */
    .promo-toolbar { display:flex; align-items:center; justify-content:space-between; margin-bottom:20px; gap:12px; flex-wrap:wrap; }
    .btn-add-promo { display:flex; align-items:center; gap:8px; padding:10px 20px; background:var(--color-primary); color:#fff; border:none; border-radius:var(--radius-xl); font-size:var(--text-sm); font-weight:var(--font-semibold); cursor:pointer; font-family:var(--font-family); transition:background var(--transition-fast); }
    .btn-add-promo:hover { background:var(--color-primary-hover); }

    /* ── Search ── */
    .table-search-wrap { position:relative; display:flex; align-items:center; width:100%; max-width:300px; height:40px; background:#fff; border:1.5px solid var(--color-border); border-radius:var(--radius-full); box-sizing:border-box; }
    .table-search-wrap:focus-within { border-color:var(--color-primary); box-shadow:0 0 0 4px rgba(139,37,0,.08); }
    .table-search-icon { position:absolute; left:14px; font-size:1rem; color:var(--color-text-hint); pointer-events:none; }
    .table-search-input { width:100%; height:100%; border:none; outline:none; background:transparent; font-family:inherit; font-size:var(--text-sm); color:var(--color-text-primary); padding:0 16px 0 40px; box-sizing:border-box; }
    .table-search-input::placeholder { color:var(--color-text-hint); }

    /* ── Table extras ── */
    .voucher-code-cell { font-family:monospace; font-weight:700; font-size:13px; color:var(--color-primary); background:rgba(139,37,0,.07); padding:3px 9px; border-radius:6px; display:inline-block; }
    .validity-cell { font-size:12px; color:var(--color-text-secondary); line-height:1.6; }
    .progress-bar-wrap { width:100%; max-width:90px; }
    .progress-bar-track { height:5px; background:#f3f4f6; border-radius:99px; overflow:hidden; }
    .progress-bar-fill { height:100%; background:var(--color-primary); border-radius:99px; transition:width .3s; }
    .progress-label { font-size:11px; color:var(--color-text-secondary); margin-top:3px; }
    .per-user-badge { font-size:11px; color:#888; margin-top:3px; }

    /* ── Modal Tambah ── */
    .modal-overlay { display:none; position:fixed; inset:0; background:rgba(0,0,0,.45); z-index:300; align-items:center; justify-content:center; backdrop-filter:blur(3px); padding:24px; box-sizing:border-box; }
    .modal-overlay.show { display:flex; }
    .modal-box { background:#fff; border-radius:16px; padding:32px; width:100%; max-width:520px; box-shadow:0 20px 60px rgba(0,0,0,.15); display:flex; flex-direction:column; gap:20px; max-height:90vh; overflow-y:auto; }
    .modal-title { font-family:var(--font-display); font-size:var(--text-xl); font-weight:700; color:var(--color-text-primary); margin:0; }
    .modal-grid { display:grid; grid-template-columns:1fr 1fr; gap:12px; }
    .modal-field { display:flex; flex-direction:column; gap:5px; }
    .modal-field.full { grid-column:1/-1; }
    .modal-label { font-size:11px; font-weight:600; color:#666; text-transform:uppercase; letter-spacing:.04em; }
    .modal-hint  { font-size:11px; color:#bbb; margin-top:2px; }
    .modal-input { padding:10px 13px; border:1.5px solid #e2e2e2; border-radius:9px; font-size:14px; font-family:inherit; outline:none; transition:border-color .15s; box-sizing:border-box; width:100%; }
    .modal-input:focus { border-color:var(--color-primary); }
    .modal-divider { height:1px; background:#f3f4f6; margin:4px 0; }
    .modal-section-label { font-size:11px; font-weight:700; color:#aaa; text-transform:uppercase; letter-spacing:.06em; }
    .modal-actions { display:flex; gap:10px; }
    .btn-modal-batal  { flex:1; padding:11px; border-radius:var(--radius-xl); border:1.5px solid #ddd; background:transparent; color:#666; font-size:14px; font-weight:600; cursor:pointer; font-family:inherit; transition:all .15s; }
    .btn-modal-batal:hover { border-color:#aaa; color:#333; }
    .btn-modal-submit { flex:1; padding:11px; border-radius:var(--radius-xl); border:none; background:var(--color-primary); color:#fff; font-size:14px; font-weight:600; cursor:pointer; font-family:inherit; display:flex; align-items:center; justify-content:center; gap:7px; }
    .btn-modal-submit:hover { background:var(--color-primary-hover); }
    .modal-error { font-size:13px; color:#c0392b; text-align:center; display:none; padding:8px 12px; background:#fff0f0; border-radius:8px; }

    /* ── Modal Konfirmasi ── */
    .confirm-overlay { display:none; position:fixed; inset:0; background:rgba(0,0,0,.45); z-index:400; align-items:center; justify-content:center; backdrop-filter:blur(4px); padding:24px; box-sizing:border-box; }
    .confirm-overlay.show { display:flex; }
    .confirm-box { background:#fff; border-radius:16px; padding:32px; width:100%; max-width:420px; box-shadow:0 20px 60px rgba(0,0,0,.18); display:flex; flex-direction:column; gap:20px; }
    .confirm-icon-wrap { width:52px; height:52px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:1.5rem; flex-shrink:0; }
    ...
    .confirm-box h3 { font-family:var(--font-display); font-size:var(--text-lg); font-weight:700; color:var(--color-text-primary); margin:0; }
    .confirm-box p  { font-size:13px; color:var(--color-text-secondary); margin:6px 0 0; line-height:1.6; }
    .confirm-actions { display:flex; gap:10px; margin-top:4px; }
    .btn-confirm-batal { flex:1; padding:11px; border-radius:var(--radius-xl); border:1.5px solid #ddd; background:transparent; color:#666; font-size:14px; font-weight:600; cursor:pointer; font-family:inherit; }
    .btn-confirm-batal:hover { border-color:#aaa; color:#333; }
    .btn-confirm-ok { flex:1; padding:11px; border-radius:var(--radius-xl); border:none; font-size:14px; font-weight:600; cursor:pointer; font-family:inherit; display:flex; align-items:center; justify-content:center; gap:7px; }
    .btn-confirm-ok.danger  { background:#dc2626; color:#fff; } .btn-confirm-ok.danger:hover  { background:#b91c1c; }
    .btn-confirm-ok.warning { background:#d97706; color:#fff; } .btn-confirm-ok.warning:hover { background:#b45309; }
    .btn-confirm-ok.success { background:#16a34a; color:#fff; } .btn-confirm-ok.success:hover { background:#15803d; }
...
<div class="admin-layout">
  ...
  <div class="main-container">
    <header class="topbar">
      <div class="topbar-left"><h1 class="page-title">Promo &amp; Deals</h1></div>
            ...
            <tbody>
              ...
              <tr data-id="<?= $p['id'] ?>"
                ...
                <td>
                  ...
                  <?php if ($p['deskripsi']): ?>
                    <div style="font-size:12px;color:#aaa;"><?= htmlspecialchars($p['deskripsi']) ?></div>
                  <?php endif; ?>
                </td>
...
document.querySelector('#promoTable tbody').addEventListener('click', function (e) {
  ...
  if (e.target.closest('.btn-toggle')) {
    ...
    showConfirm({
      ...
      msg:     `Kode "${row.dataset.kode}" akan ${nonaktif
        ? 'dinonaktifkan sehingga tidak bisa dipakai user.'
        : 'diaktifkan kembali dan bisa dipakai user.'}`,
    ...
    showConfirm({
      ...
      onOk: () => {
        ...
        fetch(window.location.pathname, { method: 'POST', body: fd })
          .then(r => r.json())
          .then(d => {
            if (d.success) { closeConfirm(); row.remove(); }
            else closeConfirm();
          });
      }
    });
