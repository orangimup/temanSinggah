document.addEventListener("DOMContentLoaded", () => {
  function closeAllDropdowns() {
    ...
    document.getElementById("guestCounterDropdown")?.classList.remove("open");
  }
  ...
  const destinationField = document.querySelector(".search-fields").children[0];
  ...
  const guestField = document.querySelector(".search-fields").children[2];
    ...
    counterItem.forEach((item) => {
      ...
      plusButton.addEventListener("click", () => {
        const isOrangGroup =
          item.dataset.group === "dewasa" || item.dataset.group === "anak";
        if (isOrangGroup) {
          if (value < max && getTotal() < 16) {
            value++;
            updateCounter();
            updateSiblingOrangGroup();
            updateGuestSummary();
          }
        } else {
          if (value < max) {
      ...
      minusButton.addEventListener("click", () => {
        if (value > min) {
          ...
          updateCounter();
          if (
            item.dataset.group === "dewasa" ||
            item.dataset.group === "anak"
          ) {
            updateSiblingOrangGroup();
          }
          updateGuestSummary();
  ...
  const dateField = document.querySelector(".search-fields").children[1];
  ...
  function updateDateSummary() {
    ...
    dateSummary.textContent = rangeEnd
    ...
    allDays.forEach((el) => {
      ...
      if (phase === "idle" && rangeStart && rangeEnd) {
        ...
        if (isSameDay(date, rangeStart)) {
          ...
          else if (date > previewStart && date < previewEnd)
